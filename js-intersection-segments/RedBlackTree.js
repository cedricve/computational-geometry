...
*
*****/
RedBlackTree.prototype.findPredecessor = function(value) {
    var result = this._root;
    var predecessor = null;
    while ( result != null ) {
        var relation = value.compare(result._value);
        if ( relation > 0 ) {
            predecessor = result._value;
            result = result._right;
        } else {
            result = result._left;
        }
...
*
*****/
RedBlackTree.prototype.findSuccessor = function(value) {
    var result = this._root;
    var successor = null;
    while ( result != null ) {
        var relation = value.compare(result._value);
        if ( relation < 0 ) {
            successor = result._value;
            result = result._left;
        } else {
            result = result._right;
        }
